...
//RCC_SYSCLKSource_HSI:内部高速时钟  //RCC_SYSCLKSource_HSE:外部高速时钟 
...
//时钟Interrupt配置。
//RCC_IT_LSIRDY:内部低速时钟基准计时器通过
//RCC_IT_LSERDY:外部低速时钟寄存器准备
//启用或禁止相应时钟中断。
...
/*
//配置I2S时钟来源。
参数RCC_I2S2CLKSource/RCC_I2S3CLKSource指定I2S2/I2S3时钟来源,可以是:
//RCC_I2S2CLKSource_PLLI2S/RCC_I2S3CLKSource_PLLI2S:PLLI2S时钟
//RCC_I2S2CLKSource_Ext/RCC_I2S3CLKSource_Ext:外部时钟
//RCC_I2S2CLKSource_PLL/RCC_I2S3CLKSource_PLL:PLL时钟(仅I2S3)
//RCC_I2S2CLKSource_NoClock/RCC_I2S3CLKSource_NoClock:没有时钟源
*/
...
/*
配置低速外部振荡器(LSE)。
参数RCC_LSE指定LSE配置,可以是:
RCC_LSE_OFF: LSE振荡器OFF
RCC_LSE_ON: LSE振荡器ON

使能或禁止内部低速振荡器(LSI)。
参数NewState指定LSI振荡器的新状态,可以是启用或禁止。

配置实时时钟(RTC)的时钟源。
参数RCC_RTCCLKSource指定RTC时钟源,可以是:
RCC_RTCCLKSource_LSE: LSE振荡器时钟
RCC_RTCCLKSource_LSI: 内部低速振荡器(LSI)时钟

使能或禁止RTC振荡器。 参见
取得参数。

填写 结构体,获取当前PLL,SYSCLK,HCLK,PCLK2和PCLK1频率。

使能或禁止外设AHB时钟。
参数RCC_AHBPeriph指定外设,可以是:
AHB外设时钟,NewState启用或禁止该时钟。

使能或禁止外设APB2时钟。 参见AHB时钟描述获取参数。

使能或禁止外设APB1时钟。 参见AHB时钟描述获取参数。
*/
#ifdef STM32F10X_CL
void RCC_AHBPeriphResetCmd(uint32_t RCC_AHBPeriph, FunctionalState NewState);// 这个函数重置指定的AHB外设
#endif /* STM32F10X_CL */ 
...
// 这个函数重置指定的APB2外设。
// RCC_APB2Periph：指定要复位的外设（可以是多个外设的组合）
// NewState：指定外设的新状态

// 这个函数重置指定的APB1外设。
// RCC_APB1Periph：指定要复位的外设（可以是多个外设的组合）
// NewState：指定外设的新状态

// 这个函数启用或禁用备份域复位。
// NewState：备份域复位的新状态。ENABLE或DISABLE。

// 这个函数启用或禁用时钟安全系统（CSS）。
// NewState：CSS的新状态。ENABLE或DISABLE。

// 这个函数配置MCO（微控制器时钟输出）引脚。
// RCC_MCO：指定MCO时钟源和分频器。
